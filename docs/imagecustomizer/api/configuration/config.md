...
- `package-snapshot-time`: Enables snapshot-based package filtering during image customization.
  When this option is specified, users can specify a RFC 3339 timestamp or ISO 8601 date
  via the `--package-snapshot-time` CLI parameter or `os.packages.snapshotTime` YAML
  configuration. The snapshot constraint affects package installations (failing if no
  eligible version exists) and updates (skipping newer versions) during customization.
