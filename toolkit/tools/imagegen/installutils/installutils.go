...
func addGroups(installChroot *safechroot.Chroot, groups []configuration.Group) (err error) {
	...
	for _, group := range groups {
		...
		err = userutils.AddGroup(group.Name, group.GID, installChroot)
		if err != nil {
			return err
		}
	}
...
func addUsers(installChroot *safechroot.Chroot, users []configuration.User) (err error) {
	...
	for _, user := range users {
		...
		err = userutils.ProvisionUserSSHCerts(installChroot, user.Name, user.SSHPubKeyPaths, user.SSHPubKeys,
