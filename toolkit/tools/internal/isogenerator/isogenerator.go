...
type IsoGenConfig struct {
	...
	// The path of the initrd file to use in ISO.
	InitrdPath string
}

type isoGenInfo struct {
	Config              IsoGenConfig
	IsoStagingDirectory string
	EfiBootImgPath      string
}

func GenerateIso(config IsoGenConfig) error {
	isoStagingDirectory := filepath.Join(config.BuildDirPath, "iso-staging")
	efiBootImgPath := filepath.Join(isoStagingDirectory, efiBootImgPathRelativeToIsoRoot)

	info := isoGenInfo{
		Config:              config,
		IsoStagingDirectory: isoStagingDirectory,
...
func applyRufusWorkaround(info isoGenInfo, bootBootloaderFile string, grubBootloaderFile string) (err error) {
	...
	buildDirBootEFIUsbFilePath := filepath.Join(info.IsoStagingDirectory, buildDirBootEFIDirectoryPath, bootBootloaderFile)
	...
	buildDirGrubEFIUsbFilePath := filepath.Join(info.IsoStagingDirectory, buildDirBootEFIDirectoryPath, grubBootloaderFile)
...
func copyInitrd(info isoGenInfo) error {
	initrdDestinationPath := filepath.Join(info.IsoStagingDirectory, info.Config.IsoOsFilesDirPath, "initrd.img")
...
func createVmlinuzImage(info isoGenInfo) error {
	...
	vmlinuzFilePath := filepath.Join(info.IsoStagingDirectory, info.Config.IsoOsFilesDirPath, "vmlinuz")
