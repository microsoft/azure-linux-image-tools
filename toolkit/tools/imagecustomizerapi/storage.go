...
type Storage struct {
	...
	// Filled in by ValidateVerityMounts() (via Storage.IsValid() or TODO).
...
func (s *Storage) IsValid() error {
	...
	verityPartitionsType := VerityPartitionsInvalid
	switch {
	...
	case verityUsesConfigPartitions:
		verityPartitionsType = VerityPartitionsUsesConfig

	case verityUsesExistingPartitions:
		verityPartitionsType = VerityPartitionsUsesExisting

	default:
		verityPartitionsType = VerityPartitionsNone
	}
	...
	if verityPartitionsType == VerityPartitionsUsesConfig && !hasDisks {
		return fmt.Errorf("cannot specify 'verity' with dataDeviceId/hashDeviceId without specifying 'disks'")
	}

	if verityPartitionsType == VerityPartitionsUsesExisting && hasDisks {
	...
	if verityPartitionsType == VerityPartitionsUsesConfig {
