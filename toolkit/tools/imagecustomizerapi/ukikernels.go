...
// UnmarshalYAML enables UkiKernels to handle both shorthand "auto" and a structured list of kernel names.
func (u UkiKernels) UnmarshalYAML(value *yaml.Node) error {
	...
	case yaml.SequenceNode:
		...
		if err := value.Decode(&kernels); err != nil {
			return fmt.Errorf("failed to decode kernel list: %w", err)
		}
		u.Kernels = kernels
		u.Auto = false
		return nil
