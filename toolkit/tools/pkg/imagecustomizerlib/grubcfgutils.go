...
func ReadGrub2ConfigFile(imageChroot safechroot.ChrootInterface, distroHandler DistroHandler) (string, error) {
	...
	grub2ConfigFilePath := filepath.Join(imageChroot.RootDir(), grubCfg.GrubCfgRelPath)
	...
	if err != nil {
		return "", fmt.Errorf("failed to read grub config file (%s):\n%w",
			grubCfg.GrubCfgRelPath, err)
	}

	return grub2Config, nil
}

// Writes the grub.cfg file.
func writeGrub2ConfigFile(grub2Config string, imageChroot safechroot.ChrootInterface,
