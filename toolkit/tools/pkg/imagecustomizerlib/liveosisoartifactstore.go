...
func createIsoFilesStoreFromMountedImage(inputArtifactsStore *IsoArtifactsStore, imageRootDir string, storeDir string) (filesStore *IsoFilesStore, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to enumerate folders under\n%w", err)
	}
	for _, kernelVersion := range kernelVersions {
		logger.Log.Infof("-- kernel version: (%s)", kernelVersion)
	}
	...
	for _, sourcePath := range bootFolderFilePaths {
		...
		logger.Log.Infof("-- ----")
		logger.Log.Infof("-- imageRootDir              = %s", imageRootDir)
		logger.Log.Infof("-- baseFileName              = %s", baseFileName)
		logger.Log.Infof("-- relativeFilePath [switch] = %s", relativeFilePath)
		logger.Log.Infof("-- sourcePath                = %s", sourcePath)
		logger.Log.Infof("-- filesStore.artifactsDir   = %s", filesStore.artifactsDir)
		logger.Log.Infof("-- targetPath                = %s", targetPath)
		...
		for _, kernelVersion := range kernelVersions {
			// Is it a kernel specific file?
			if !strings.Contains(baseFileName, kernelVersion) {
				continue
			}
			...
			if strings.HasPrefix(baseFileName, vmLinuzPrefix) {
				logger.Log.Infof("-- debug -- store found vmlinux (%s)", targetPath)
				...
				// Old behavior - shorten the name to 'vmlinuz'.
				...
				logger.Log.Infof("-- debug -- store found initramfs (%s)", targetPath)
				kernelBootFiles.initrdImagePath = targetPath
				scheduleAdditionalFile = false
			} else {
				logger.Log.Infof("-- debug -- store found other (%s)", targetPath)
				kernelBootFiles.otherFiles = append(kernelBootFiles.otherFiles, targetPath)
				scheduleAdditionalFile = false
			}
		}

		// logger.Log.Infof("-- debug -- copying to store (%s) to (%s)", sourcePath, targetPath)
...
func dumpFilesStore(filesStore *IsoFilesStore) {
	...
	logger.Log.Infof("-- vmlinuzPath              = %s", fileExistsToString(filesStore.vmlinuzPath))
