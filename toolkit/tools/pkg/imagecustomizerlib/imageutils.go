...
func connectToExistingImageHelper(imageConnection *ImageConnection, imageFilePath string,
	...
	/*
		...
		localRepo := "/home/himaja/azurelinux/toolkit/pkgrepo/"
		repoFile := "/home/himaja/azure-linux-image-tools/toolkit/resources/manifests/image/local.repo"
		// Create setup chroot
		...
		extraMountPoints := append(mountPoints, additionalExtraMountPoints...)
	*/

	err = imageConnection.ConnectChroot(imageChrootDir, false, nil, mountPoints, includeDefaultMounts, tarFile)
	if err != nil {
		return nil, err
	}
...
func createNewImageHelper(targetOs targetos.TargetOs, imageConnection *ImageConnection, filename string,
	...
	imageFstabFilePath := filepath.Join(imageConnection.Chroot().RootDir(), "installroot/etc/fstab")
