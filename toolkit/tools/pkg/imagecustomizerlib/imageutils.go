...
) (*ImageConnection, map[string]diskutils.FstabEntry, map[string]verityDeviceMetadata, error) {
	imageConnection := NewImageConnection()

	partUuidToMountPath, verityMetadata, err := connectToExistingImageHelper(imageConnection, imageFilePath, buildDir, chrootDirName, includeDefaultMounts, onlyAddFiles)
	if err != nil {
...
func connectToExistingImageHelper(imageConnection *ImageConnection, imageFilePath string,
	...
	mountPoints, partUuidToFstabEntry, verityMetadata, err := fstabEntriesToMountPoints(fstabEntries, partitions, buildDir, onlyAddFiles)
	if err != nil {
