...
// Determines the boot configuration type used by the image.
// Returns the boot config type (grub-mkconfig, grub-legacy, or uki).
func getBootConfigType(imageChroot *safechroot.Chroot) (bootConfigType, error) {
	grub2ConfigFile, err := ReadGrub2ConfigFile(imageChroot)
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			// If grub.cfg doesn't exist, check for UKI files
			hasUkis, ukiErr := baseImageHasUkis(imageChroot)
			if ukiErr != nil {
				return "", fmt.Errorf("failed to check for UKI files:\n%w", ukiErr)
			}
			if hasUkis {
				return bootConfigTypeUki, nil
			}
			// No grub.cfg and no UKIs - this is an error state
	...
	if isGrubMkconfigConfig(grub2ConfigFile) {
