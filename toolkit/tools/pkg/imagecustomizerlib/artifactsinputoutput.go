...
func outputArtifacts(ctx context.Context, items []imagecustomizerapi.OutputArtifactsItemType,
	...
	if err != nil {
		return fmt.Errorf("%w (partition='%s'): %w", ErrArtifactESPPartitionMount, systemBootPartition.Path, err)
	}
	...
	if slices.Contains(items, imagecustomizerapi.OutputArtifactsItemUkis) {
		...
		if err != nil {
			return fmt.Errorf("%w (directory='%s'): %w", ErrArtifactUKIDirectoryRead, ukiDir, err)
		}
...
func writeInjectFilesYaml(metadata []imagecustomizerapi.InjectArtifactMetadata, outputDir string) error {
	...
	if err != nil {
		return fmt.Errorf("%w: %w", ErrArtifactInjectFilesYamlMarshal, err)
	}
...
func InjectFilesWithConfigFile(ctx context.Context, buildDir string, configFile string, inputImageFile string,
	...
	if err := injectConfig.IsValid(); err != nil {
		return fmt.Errorf("%w: %w", ErrArtifactInvalidInjectFilesConfig, err)
	}
	...
	if err != nil {
		return fmt.Errorf("%w (path='%s'): %w", ErrArtifactInjectFilesPathResolution, baseConfigPath, err)
	}
...
func InjectFiles(ctx context.Context, buildDir string, baseConfigPath string, inputImageFile string,
	...
	for idx, item := range metadata {
		...
		if _, exists := partitionsToMountpoints[partitionKey]; !exists {
			...
			if err != nil {
				return fmt.Errorf("%w (partition='%s'): %w", ErrArtifactInjectFilesPartitionMount, partition.Path, err)
			}
			defer mount.Close()
	...
	for _, m := range mountedPartitions {
		if err := m.CleanClose(); err != nil {
			return fmt.Errorf("%w (target='%s'): %w", ErrArtifactPartitionUnmount, m.Target(), err)
		}
	}
...
func prepareImageConversionData(ctx context.Context, rawImageFile string, buildDir string,
	...
	if err != nil {
		return nil, nil, "", nil, [randomization.UuidSize]byte{}, "", nil, fmt.Errorf("%w: %w", ErrArtifactImageConnectionForExtraction, err)
	}
	...
	if err := imageConnection.CleanClose(); err != nil {
		return nil, nil, "", nil, [randomization.UuidSize]byte{}, "", nil, fmt.Errorf("%w: %w", ErrArtifactImageConnectionClose, err)
	}
...
func extractImageUUID(imageConnection *imageconnection.ImageConnection) ([randomization.UuidSize]byte, string, error) {
	...
	if err != nil {
		return emptyUuid, "", fmt.Errorf("%w (path='%s'): %w", ErrArtifactReleaseFileRead, releasePath, err)
	}
	...
	if err != nil {
		return emptyUuid, "", fmt.Errorf("%w (IMAGE_UUID='%s'): %w", ErrArtifactImageUuidParse, uuidStr, err)
	}
