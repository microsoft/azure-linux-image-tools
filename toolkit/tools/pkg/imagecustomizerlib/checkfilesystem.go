...
func checkFileSystem(fileSystemType string, path string) error {
	...
	case "ext2", "ext3", "ext4":
		...
		if err != nil {
			return fmt.Errorf("%w (path='%s'): %w", ErrFilesystemE2fsckCheck, path, err)
		}
	...
	case "xfs":
		...
		if err != nil {
			return fmt.Errorf("%w (path='%s'): %w", ErrFilesystemXfsRepairCheck, path, err)
		}

	default:
		err := shell.ExecuteLive(true /*squashErrors*/, "fsck", "-n", path)
		if err != nil {
			return fmt.Errorf("%w (path='%s'): %w", ErrFilesystemFsckCheck, path, err)
		}
	}
