...
func resizeDiskAndPartitionsHelper(loopback *safeloopback.Loopback, buildDir string,
	...
	currentFreeSpace, err := getPartitionFreeSpace(buildDir, lastPartition)
	if err != nil {
		return err
	}
...
func getPartitionFreeSpace(buildDir string, partition diskutils.PartitionTablePartition) (uint64, error) {
	mountDir := filepath.Join(buildDir, "tmp-last-partition")

	mount, err := safemount.NewMount(partition.Path, mountDir, partition.FileSystemType, unix.MS_RDONLY, "", true)
	if err != nil {
		return 0, err
	}
	defer mount.Close()
	...
	if err != nil {
		return 0, fmt.Errorf("failed to read partition stats:\n%w", err)
	}
