...
func refreshTdnfMetadata(ctx context.Context, imageChroot *safechroot.Chroot, toolsChroot *safechroot.Chroot) error {
	...
	if err != nil {
		return NewImageCustomizerErrorWithCause(ErrPackageManagement, "failed to refresh tdnf repo metadata", err)
	}
	return nil
}
...
func collectPackagesList(baseConfigPath string, packageLists []string, packages []string) ([]string, error) {
	...
	for _, packageListRelativePath := range packageLists {
		...
		if err != nil {
			return nil, NewImageCustomizerErrorWithCause(ErrFilesystemOperation, fmt.Sprintf("failed to read package list file (%s)", packageListFilePath), err)
		}
...
func updateAllPackages(ctx context.Context, imageChroot *safechroot.Chroot, toolsChroot *safechroot.Chroot) error {
	...
	if err != nil {
		return NewImageCustomizerErrorWithCause(ErrPackageManagement, "failed to update packages", err)
	}
