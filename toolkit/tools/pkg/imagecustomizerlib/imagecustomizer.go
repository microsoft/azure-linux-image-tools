...
func convertWriteableFormatToOutputImage(ic *ImageCustomizerParameters, inputIsoArtifacts *LiveOSIsoBuilder) error {
	...
	case ImageFormatIso:
		if ic.customizeOSPartitions || inputIsoArtifacts == nil {
			selinuxModeConfig := imagecustomizerapi.SELinuxModeDefault
			if ic.config.OS != nil {
				selinuxModeConfig = ic.config.OS.SELinux.Mode
			}
			err := createLiveOSIsoImage(ic.buildDir, ic.configPath, inputIsoArtifacts, selinuxModeConfig, ic.config.Iso, ic.config.Pxe,
