...
$(TOOL_BINS_DIR)/%: $(go_common_files) $(license_scan_dependency)
	cd $(TOOLS_DIR)/$* && \
		go test -v -ldflags="$(go_ldflags)" -test.short -covermode=atomic -coverprofile=$(BUILD_DIR)/tools/$*.test_coverage ./... && \
		CGO_ENABLED=0 go build \
			-ldflags="$(go_ldflags)" \
			$(if $(filter y,$(BUILD_TOOLS_NONPROD)),,-tags prod) \
			-o $(TOOL_BINS_DIR)

# Runs tests for common componentsgo test
$(BUILD_DIR)/tools/internal.test_coverage: $(go_internal_files) $(go_imagegen_files) $(STATUS_FLAGS_DIR)/got_go_deps.flag
	cd $(TOOLS_DIR)/$* && \
		go test -v -ldflags="$(go_ldflags)" -test.short -covermode=atomic -coverprofile=$@ ./...
...
$(BUILD_DIR)/tools/all_tools.coverage: $(call shell_real_build_only, find $(TOOLS_DIR)/ -type f -name '*.go') $(STATUS_FLAGS_DIR)/got_go_deps.flag
	cd $(TOOLS_DIR) && go test -v -ldflags="$(go_ldflags)" -coverpkg=./... -test.short -covermode=atomic -coverprofile=$@ ./...
